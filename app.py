...
import pandas as pd  # <--- Member 2's primary tool
...
def main():
    ...
    # Fetch raw data from Member 1's logic
    raw_data, info = get_stock_data(symbol, time_range)

    if raw_data is not None:
        # 1. Member 2 explicitly creates a DataFrame using pd
        data = pd.DataFrame(raw_data) 
        
        # 2. Member 2 uses pd to ensure the index is in datetime format
        data.index = pd.to_datetime(data.index)
        
        # 3. Member 2 calculates a simple indicator (Moving Average)
